...
export function CtaButton({cta, tone = 'dark'}: CtaButtonProps) {
  ...
  const styles =
    tone === 'light'
      ...
      : tone === 'outline'
        ? {bg: 'transparent', borderWidth: '1px', borderColor: 'ink.900', color: 'ink.900', _hover: {bg: 'ink.900', color: 'ivory.50'}}
  ...
  if (isExternal) {
    return (
      <Button asChild borderRadius="full" fontWeight="600" px="6" {...styles}>
        <a href={cta.href} rel={cta.href.startsWith('http') ? 'noreferrer' : undefined} target={cta.href.startsWith('http') ? '_blank' : undefined}>
          {label}
        </a>
      </Button>
    )
